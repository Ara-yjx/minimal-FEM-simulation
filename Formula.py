...
MU = 0.3
LAMBDA = 0.3
GAMMA = 8 # damping coefficient
MASS = 20 # smaller mass, more dramatic change in deformation
...
def update_XV(X, T, V, B, W, DELTA_TIME):
    Fe = compute_F(X, T, B, W)
    # Fd = - GAMMA * K(X*)V* = - GAMMA * df(dX=V)
    Fd = - GAMMA * compute_dF(X, negative_V(V), T, B, W)
    F = Fe + Fd
    ...
    for i in range(len(X)):
        ...
        # x' = x + dt * v
        X[i] = add(X[i], DELTA_TIME * V[i])

        # Gound collision
        if X[i][2] < 0: # if a node collides with the ground
            X[i][2] = 0  # it will be forced to move above the ground
